...
* Understand the meaning of Detection rules.
* Learn about types of Detection rules.
...
6. Select the **Metric Compartment** where the metrics must be stored.
...
10. Click **Create detection rule**.
...
6. In the **Metric description** area, enter values to specify the metric to evaluate for the alarm.
    ...
    4. **(Optional) Resource group:** Select the group that the metric belongs to. A resource group is a custom string provided with a custom metric, and is not applicable to service metrics.
    5. **Metric name:** Select the name of the metric that you want to evaluate for the alarm. Let;s say you selected **"Livelab_metricname"**, which you created while creating detection rule.
    6. **Interval:** Select the aggregation window, or the frequency at which data points are aggregated. You can create a custom interval if needed.
    7. **Statistic**: Select the function to use to aggregate the data points.
...
7. In the **Metric dimensions** area, specify optional filters to narrow the metric data that's evaluated.
    - **Dimension name:** Select a qualifier that's specified in the metric definition.
